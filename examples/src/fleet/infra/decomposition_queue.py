...
4. **When it is worth it, measured.** On the repository's own decomposition
   ...
   sub-solves to dominate the pool, which is what §7.7 is about.
...
from vrp.model import Order, Problem, StopSpec, TimeWindow, Vehicle
...
def a_bigger_instance(orders: int, vehicles: int) -> Problem:
    """A generated round large enough for a cluster to be real work."""
    coords = fixtures.scatter(orders, seed=7, spread_km=40.0, clusters=6)
    day = TimeWindow(start=0, end=14 * HOUR)
    return Problem(
        id=f"big-{orders}",
        locations=fixtures.planar_sites(((0.0, 0.0), *coords)),
        orders=tuple(
        ...
        vehicles=tuple(
            Vehicle(id=f"V{i}", capacities={"kg": 1000}, shift=day,
                    start_location_id="D", end_location_id="D")
            for i in range(1, vehicles + 1)),
        matrix=fixtures.planar_matrix("big", ((0.0, 0.0), *coords)))
...
def when_it_is_worth_it() -> None:
    ...
    for orders, vehicles, target in ((300, 20, 40), (600, 30, 60)):
        problem = a_bigger_instance(orders, vehicles)
        cases.append((f"{orders} orders, generated", problem,
