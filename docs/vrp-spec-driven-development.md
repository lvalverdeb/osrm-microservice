...
| `T-83` | blocked | Per-arc (or per-zone) speed profiles in the matrix, so a per-arc-class calibration has somewhere to land | T-63 | §6.3, MTX-9, §12.2 | An instance carries a profile per arc class rather than one for the whole problem; `MTX-9`'s `O(nnz · T)` storage is what is built; `SpeedCalibration.profile` stops being the only way to apply a fit |
