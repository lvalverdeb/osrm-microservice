...
| `T-85` | open | Insertion and removal quotes, priced and timed | T-33, T-56 | NFR-02 (first clause), §7.3 | A single-order insertion or removal returns a priced quote at p95 ≤ 2 s on the reference instance, measured the way `tests/vrp/test_reoptimisation_latency.py` measures the second clause. The pricing exists in `vrp.lns`'s greedy insertion; what is missing is a quote path that does not re-solve, and a timing harness |
| `T-86` | open | Bounded intra-run parallelism, and isolation somebody has checked | T-36, T-37 | NFR-05, §7.7 | `run_portfolio` runs its engines in one `for` loop today. §7.7 asks for portfolio members on separate cores and decomposition sub-problems in a work queue, with shared state limited to the incumbent pool; and CON-4's reproducible mode MUST stay single-threaded and iteration-limited. Done when two runs of the same instance on separate workers cannot observe each other, parallel and serial portfolios return the same winner under a fixed seed, and the speed-up is reported rather than assumed |
| `T-87` | open | The run record `NFR-06` actually asks for | T-15, T-36 | NFR-06, CON-4 | `Solution.solver` records solver, seed, iterations and matrix version — four of the seven things `NFR-06` names. Missing: objective trajectory over time, incumbent timestamps, constraint-violation counts, and matrix cache hit rate (the gateway counts cache lookups; the solve does not see them). Done when one run emits all seven from one place and a test asserts each is present and moves when the thing it measures moves |
...
| `T-89` | open | A plan replayable from its own immutable snapshot | T-15, T-53 | NFR-08, CON-4 | `vrp/replay.py` replays historical *days* for policy evaluation (DYN-6), which is a different thing. `NFR-08` wants the input snapshot, solver configuration and output plan retained immutably, and a plan reproducible from its snapshot. `Solution.solver` makes that possible in principle and nothing stores the snapshot. Done when a plan is re-derived byte-for-byte from its recorded snapshot alone, and a mutated snapshot is refused rather than replayed |
| `T-90` | open | Version the public API before something depends on the unversioned one | T-15 | NFR-10, §9.4 | The gateway serves `/route`, `/vrp`, `/matrix` and the rest unversioned (`gateway/src/main.rs`), so there is no path to a breaking change that keeps a promise. Done when the surface is served under a major version, the unversioned paths keep working for a stated deprecation window, and a test pins both. **Rust gateway**, not `vrp/`: it is the contract the HTTP surface makes |
