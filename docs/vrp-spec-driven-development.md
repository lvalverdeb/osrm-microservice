...
| `T-102` | open | Make `vrp` an installable distribution | T-101 | NFR-08, CON-6 | `vrp` is 64 modules importable only because `pytest` puts the repo root on `pythonpath`; there is no `[build-system]`, no wheel, and `uv.lock` records the root as `virtual`. Nothing outside this checkout can depend on it at a version. Four modules resolve paths to the **repo root** and read files outside the package -- `servicemodel` (`models/`, now mitigated by `T-101`), `bench/catalogue` (`docs/TDD/scenarios.jsonl`), `bench/comparable` (`benchmarks/instances/`) and `anonymise` (`data/`) -- so an installed copy would find none of them. Done when a built wheel imports and solves without the checkout present, `vrp/bench` is either excluded or its data shipped with a stated reason, and `uv sync --extra dev` still does what CI expects -- that last one verified locally before it is pushed, because flipping the root from `virtual` to a real package changes the exact command CI runs |
