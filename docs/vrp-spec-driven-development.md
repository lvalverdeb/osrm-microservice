...
| `T-91` | open | Process-based parallelism for engines that hold the GIL | T-86, T-89 | NFR-05, §7.7 | `T-86` measured 1.00x for the repository's own LNS across four threads: pure-Python members take turns at one interpreter, so §7.7's "members on separate cores" holds for the C++ engines and not for them. A process pool is what would fix it, and `vrp.snapshot`'s canonical encoding (`T-89`) is how a problem gets to a worker. Done when a pure-Python member measures a speed-up above 1 across processes, the winner and the report are unchanged, and CON-4's single-worker mode still runs inline |
| `T-92` | open | Decomposition sub-problems in a work queue | T-37, T-86 | NFR-05, §7.7 | §7.7 asks for two kinds of intra-run parallelism and `T-86` delivered one. `vrp.decompose` still solves its sub-problems in sequence, which is the larger of the two on a 10,000-stop instance because there are more sub-problems than portfolio members. Done when sub-problems run through a bounded queue, the plan is unchanged under a fixed seed, and the speed-up is measured on `uc074` rather than assumed |
...
| `T-86` | done | Bounded intra-run parallelism, and isolation somebody has checked | T-36, T-37 | NFR-05, §7.7 | `run_portfolio(..., workers=N)` runs members in a bounded thread pool, defaulting to one because CON-4's reproducible mode is single-threaded and a library that parallelised unasked would make every existing caller irreproducible. Concurrency is *proven* by a barrier two engines cannot both reach unless they are in flight together — a `workers` argument that is accepted and ignored passes every same-answer test and fails that one. The report is collected in portfolio order, so two runs serialise identically. **Speed-up measured, and it is engine-dependent: 3.00x at four workers for PyVRP, 1.00x for the repository's own pure-Python LNS.** §7.7's "separate cores" holds only for engines that release the GIL; the rest is `T-91`. Decomposition sub-problems still run serially — see below |
