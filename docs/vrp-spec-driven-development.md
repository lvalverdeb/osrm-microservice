...
| `T-94` | open | Delivery models: an operation described as data | T-89 | FR-02, FR-04, FR-05, NFR-08 | In-repo JSON describes a model -- vehicle, unit, service, windows, depot strategy, route closure -- resolved through `vrp/servicemodel.py` into a `Problem`, and mapped to item categories. Named strategies only: a model *names* a shipped computation and never describes one, and cannot contain anything needing a solve to define itself. Completeness is tested by reflection over `dataclasses.fields`, not a hand-written list, because a hand-written one is how `_vehicle_from_dict` reconstructed 9 of 28 fields until `T-89`; the throwaway spike managed 8 of 28 and would have shipped the same defect. The resolved model's hash goes in the snapshot or a plan stops being replayable the moment a file is edited. Composition is by disjoint named fragments with exactly one level of deployment overlay over a declared tunable surface. See `docs/planning/DELIVERY_MODELS_PLAN.md` |
