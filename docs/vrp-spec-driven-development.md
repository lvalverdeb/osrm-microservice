...
- `INV-16` An electric vehicle never arrives anywhere past empty, and charges only where its own `charger_locations` say there is a charger
  (`FR-26`).
- `INV-17` No route departs before every order aboard it is released (`FR-06`). Measured at the
  route's departure, not at the stop: a release time is a fact about the depot, so the constraint
  binds when the vehicle leaves carrying the goods.

**`INV-10`–`INV-17` are numbered past §4.3's original nine deliberately.** Each was added when a
...
- Checks INV-1 … INV-17 and every lock.
...
rather than counting the proxy. `blocked` and `optional` are the two that remain:

| ID | Why it is open |
|---|---|
| `T-99` | **Unblocks when** a live operation supplies delivery records. Like `T-84` this is about the outside world rather than about effort, but for the opposite reason: `T-84` waits on a machine and this waits on customers. Building its fixtures early is not neutral -- §13.4 says production instances *replace* the synthetic ones, so synthetic `P1`/`P2` fixtures would be work done to be discarded, reported as coverage in the meantime. `T-88` removed the part that was a real gap, which was having nowhere safe to write such a corpus |
...
Neither of the remaining two is blocked on effort. A status column goes stale the moment somebody forgets to
...
| `T-04` | done | **[GATE]** Independent verifier package (§11.2) | T-02 | CON-1, INV-1…INV-9 (extended to INV-17 by later tasks) | Separate package, separate author; detects seeded violations in 100% of mutation tests |
...
| ID | Status | Task | Deps | Satisfies | Definition of done |
|---|---|---|---|---|---|
| `T-10` | done | OSRM adapter: table + route, snapping, unreachable sentinels | T-02 | MTX-1…MTX-5 | Integration test against a local OSM extract. **Its `MTX-1` claim was partial and stayed that way for months:** the profile reached the matrix version and the cache key, which is real, and there was never a second profile to key -- one graph was built, one `osrm-routed` served it, and OSRM ignores the profile in a request path, so `driving`, `cycling` and `walking` returned identical routes. `T-98` delivered the rest. A requirement claimed by a done task is not thereby met, which is the lesson worth keeping from this row |
...
| `T-88` | done | An anonymisation gate for anything leaving the production boundary | T-11 | NFR-07 | The corpora in `vrp/bench` are generated, so no address has ever left; that is a property of where the fixtures came from rather than a control. `vrp/anonymise.py` is the control: `write_corpus` refuses records still carrying `order_id`/`product_id` **and names them**, and refuses records stripped by hand -- hand-stripping leaves the coordinate at the front door, and the type is the only thing that can tell the difference. The gate re-reads the content rather than trusting the type it was handed, because a gate that trusts a type trusts whoever constructed it. Fields are classified the way `T-94` classifies `Vehicle`'s: `IDENTIFYING`/`COARSENED`/`SAFE` account for all 12 corpus columns with a reason each, checked against what the generator writes, so a new column fails the suite until somebody decides about it. **The grid is derived, not rounded**: median nearest-neighbour spacing over all 50,000 shipped stops is 66 m, so a 50 m grid displaces a stop at most 35 m and never past its nearest neighbour. `MEDIAN_SPACING_METRES` pins that at 60 for headroom, and one test checks the grid against the pin while another checks the pin against whatever corpus is on the machine, so it cannot go stale in either direction. Coarsening was measured and rejected -- at 500 m, 73% of stops collapse onto a shared point and 14% are *still* alone in their cell, which costs the benchmark its geometry and buys little. **Stated limit:** stripping the identifiers is the guarantee; moving the coordinate defeats the doorstep, not a determined match against a known address list, and no resolution rescues the isolated rural delivery. Seven perturbations: letting identity through, passing an unclassified field, leaving coordinates where collected, ignoring the salt, widening the grid past the spacing it preserves, moving the pin down to meet a denser corpus, and accepting hand-stripped records -- each caught. The grid one twice over: the first draft of its test measured a *thinned* corpus and passed at 500 m, and the second read `data/deliveries_cr.json`, which is generated and gitignored -- green locally, `FileNotFoundError` on CI, and had it merely skipped there CI would have been validating the grid against the committed slice, whose stops sit 89 m apart rather than 66 m. That is how a 100 m grid passes review. §13.4's anonymised production instances stay out of scope: that directive is conditional on customers arriving, and there are none |
...
| `T-95` | done | The run configuration a model does not own | T-94, T-13 | §5.1, §5.3, NFR-08 | `ObjectiveSpec`'s mode and rates, the engine, its budget and its seed are not in `Problem`, so a model that pins a fleet has not pinned a plan. `servicemodel.run_config` resolves a `run` section into a `RunConfig`, and its absence is an error rather than a default -- an invisible load-bearing default would let two readers of one file disagree about which plan it describes with neither of them wrong. Engines and modes are named against registries that list what ships. The budget is spelled `budget`, not `iterations`, because the adapters disagree: PyVRP takes `iterations` and OR-Tools `solutions`, and a model should name a quantity rather than one engine's parameter. `as_config` gives `snapshot.capture` the model's name **and** the digest of its resolved form -- a name alone goes stale when the file is edited, a digest alone cannot be looked up, and hashing the resolved model rather than the file means two files meaning the same thing seal alike. `ObjectiveSpec` joins the `T-94` contract, so a field added to it fails the suite until somebody decides about it. **Correcting this row's own acceptance:** it asked for two models to produce different *plans* from identical demand, and that is not achievable -- no solver adapter accepts an `ObjectiveSpec`. The spec is consumed by `objective`, `evaluator` and `allocate`, all of which score a plan that already exists, so a mode decides which of two plans wins rather than which one an engine finds. Demonstrated that way instead, and the limitation is pinned by a test that fails if an adapter ever takes one. Anything dynamic remains out of scope |
| `T-96` | done | The gate that makes model-driven configuration safe | T-94, T-95, T-46, T-93, T-65 | FR-34, §11.4, AC-4.2 | Moving configuration from code to JSON moves a class of failure from review time to run time; `vrp/modelcheck.py` buys it back. `structural` is solve-free and pre-commit shaped; `check` runs a model over one round and `compare` runs several over the *same* demand, because two models measured on two rounds are not comparable and the difference would read as a property of the models. **Only verifier-accepted plans score** -- a rejected plan carries no cost and no binding constraint, and the test for it uses a *solved* refusal rather than one refused at load, because a load-time refusal says nothing about the gate. Attainment rather than `MixResult.service_level`, which counts placement and would score a model serving everything two hours late as perfect. **Which constraint bound** is the new capability: slack as a share of each constraint's own budget, so shift-seconds and box-kilograms rank against one another and the tightest wins. Measured on the shipped models over corpus demand: `signed-envelopes` binds on **shift** with 291ppt spare, `mixed-parcels` on **fleet** with 666ppt. Named `modelcheck` rather than the proposed `compare`, which would have collided with `objective.compare`. **Not delivered:** validating *composite* models -- disjoint sections, no chains, an overlay inside its tunable surface, attributing a variant's failure to one overlay field -- needs composition, which is not built. That is `T-97` |
| `T-97` | done | Model composition: fragments, overlays and a tunable surface | T-94, T-96 | NFR-08 | Six cities differing slightly is six near-identical model files, which is the repetition delivery models exist to remove. A master composes *sections* from named fragments, each claimed by exactly one source, so overlap is a conflict the loader refuses rather than something it resolves by order -- that removes the diamond problem instead of ordering it. Exactly one level of deployment overlay, over fields the master declares `tunable`, which makes a master a contract: a city may raise its courier count and may not quietly change the capacity its feasibility depends on. Lists addressed by identity rather than position. Every resolved field carries its provenance, and the gate runs per variant rather than per master -- a validated master shipping six unvalidated variants is the failure this design exists to prevent. Depth one is what makes `T-96`'s failure attribution possible at all: `modelcheck.attribute` reverts each overlay field in turn and is bounded only because the overlay is one level deep; with chained variants the field to revert could live in any ancestor and the answer would be a path through files rather than a field. Delivered as `servicemodel.resolve`/`resolve_model` returning `Resolved(model, provenance)`, plus `shipped()` -- the gate runs over *that* list, masters and variants alike, which is what makes "per variant" real rather than intended. An overlay may change a value and may not introduce one, so a typo is a failed override rather than a setting nobody reads. Categories map to *masters*: a variant is selected by where it runs rather than by what is being delivered, so `unreachable` counts it reachable through its base -- a throwaway variant shipped against the real `models/` found that the both-directions check would otherwise have failed the first variant to land, calling it dead weight while it was anything but. Eight perturbations: ignoring the `tunable` surface, inventing a field, positional list addressing, chained variants, gating the master instead of the variant, blaming the first overlay field, `shipped()` listing only masters, and reachability ignoring the base -- each was caught. Plan: `docs/planning/DELIVERY_MODELS_PLAN.md` |
| `T-98` | done | One routing graph and one engine per profile | T-10, T-90 | MTX-1, FR-07 | `MTX-1` for real, through all three layers. The gateway holds an upstream per profile (`OSRM_URL_DRIVING`/`_CYCLING`/`_WALKING`, driving falling back to `OSRM_BASE_URL`) and refuses a profile it has no graph for by name -- a 422 saying which it serves -- rather than answering from the driving engine. The profile is read from the upstream path rather than passed beside it, so a handler cannot build one profile's URL and send it to another's engine. Both deployments build a graph per profile and run an engine per graph: compose gains two services, and the jail runs one multi-instance rc.d script installed under three names, with a data phase that builds only what is missing, one profile at a time, because that box has 2 GB and `osrm-extract` is already flagged memory-hungry on it. **Measured through the deployed gateway** on the same two points: driving 3,911 m / 410 s, cycling 3,745 m / 1,073 s, walking 3,963 m / 2,872 s. Walking is the proof -- *longer* than driving in distance, because pedestrians take different links, and seven times the duration; a car graph with a slower constant cannot produce that. **Breaking change:** callers of `cycling` or `walking` were getting car routing and now get a refusal where no graph is configured. Two defects were caught by proving the risky script on a copy before the real path touched it: `start_precmd` resolving per-instance would have failed the precmd for every engine including the live one, and the `PROFILES` validator initially rejected the shipped default because an empty rc.d suffix matched the gap between any two entries |
| `T-99` | blocked | `P1`/`P2` fixtures, as the production instances §13.4 asks for | T-88, T-65, T-53 | §13.4, §13.5, NFR-07 | The coverage report reads `P1 0/49`, `P2 0/79` and looks like 128 missing fixtures. It is not: §13.1 asks for *synthetic* fixtures for `P0` only, and those are delivered 14/14 at three sizes. §13.4 says `P1` and `P2` scenarios "become anonymised production instances **as customers arrive**, replacing their synthetic equivalents" -- so generating 128 synthetic fixtures now would build precisely the thing the directive says to throw away, and would read as coverage while measuring the generator's imagination. Filed rather than left untracked because a `0%` line nobody has explained is indistinguishable from a `0%` line nobody has noticed. **Unblocks when** a live operation supplies delivery records; the prerequisite that was genuinely missing is gone, since `T-88` put the anonymisation gate on the write. Done when each `P1`/`P2` scenario a real operation exercises has a fixture derived from its deliveries through `vrp.anonymise`, each carrying its `Breaks` assertion as a named test (§13.5), and the report stops reading `0%` for the tiers that have data. Not done by inventing demand: `vrp/bench/fixtures.py` builds `P0` from seeds because `P0` is "must work at v1" and cannot wait, and §13.4 is the paragraph that says the rest can |
| `T-100` | done | Tie each variant to a benchmark-comparable fixture, or say why it has none | T-06, T-53 | §13.3, §11.3, FR-01 | §13.3 is unconditional -- "each variant section contributes at least one benchmark-comparable fixture so public benchmark performance and production performance can be related" -- and nothing checks it. Unlike `T-99` this waits on nobody: `T-06` shipped the readers, 11 public instances are committed under `benchmarks/instances/`, and `vrp/bench/fixtures.py` holds the synthetic side. What is missing is the *link*: no data anywhere says which instance makes `CVRP`'s fixtures comparable to CVRPLIB, so the relation §13.3 exists to permit cannot be drawn, and its absence is invisible. The nine variants are not equal and the task must not pretend they are -- `TSP`/`CVRP`/`VRPTW`/`MDHVRPTW`/`PDPTW` are `adequate`, `DARP` is `thin`, and `IRP`/`CARP`/`LRP` are `deliberately partial`, so a variant with no public set to compare against is a legitimate answer and an unexplained one is not. Done when each variant in §2's index names either a benchmark-comparable fixture or a written reason, as data rather than prose, and a test fails on a variant carrying neither -- `test_every_adversarial_instance_has_a_fixture_or_a_written_reason` and `NOT_AN_INSTANCE` are the idiom to copy, because they already solved this shape for §11. Note the objective conventions differ per set (§11.3), so "comparable" means the gap is computed the way the set defines it, not the way our objective does. **Delivered as `vrp/bench/comparable.py`**: `ANCHOR` for the five advertised variants, `NO_ANCHOR` with a read reason for the four beyond them, and `DEFINING` -- the structural facts an anchor must *exhibit*, read out of the crossed `Problem` rather than the file's `TYPE` line. **The gate found a real defect on its first run.** `lrc206.vrp` is the only `PDPTW` anchor, twenty-five scenarios' worth, and it was crossing as 102 independent jobs: `read_benchmark` never looked at `PICKUP_AND_DELIVERY_SECTION`, so Li & Lim's precedence and same-vehicle rule were silently absent and any gap against its published best-known would have measured a strictly easier problem while looking fine -- the exact failure `benchmarks/instances/README.md` says `PR01` exists to keep tested, on a file nothing was testing it on. Fixed rather than excused, because recording it as a `NO_ANCHOR` reason would have laundered a bug into an accepted absence: `_shipments` maps the section to `SHIPMENT` orders (`FR-01`) and `_pairing_offset` *derives* the node-numbering offset by requiring the pairing to be symmetric -- `lrc206` numbers its nodes 0, 2, 3 ... 103 with no node 1, and a mispaired shipment reads as a valid instance answering a different problem. It now solves FEASIBLE, verifier-accepted, 51 shipments each visited twice on one route. `DARP`'s entry is marked in the module as the one reason that is a to-do rather than a position: Cordeau & Laporte's instances are public, unvendored, and `max_ride_time` exists on our side. Six perturbations: anchoring every variant at one convenient file, excusing an advertised variant, naming a file that is gone, reducing a reason to a placeholder, dropping the pairing again, and accepting a pairing offset without checking symmetry -- each caught |
| `T-101` | done | Models a deployment ships, not only the ones this repository does | T-94, T-96, T-97 | FR-02, NFR-08, §11.4 | `servicemodel` calls a delivery model "an operation described as data", which is only true if an operation this repository has never heard of can be described. `MODELS` was one directory resolved against the checkout, so the capability stopped at the repo boundary -- and stops altogether once `vrp` is installed rather than run from a tree. Models are now looked up along a **path**, nearest first, prepended through `VRP_MODEL_PATH`; `model_for`, `shipped`, `fragment_for` and the category map all walk it. The maps **merge** rather than the nearest file winning outright, so adding one category does not cost a deployment every category we already mapped, while a redirect still redirects -- and that pair of properties is what the ordering test pins, because reversing it breaks only the second and breaks it silently. Read at call time, not at import: a process that configures itself in `main()` is the normal case, and the composition tests set `MODELS` directly. **`T-96`'s gate is not weakened** -- it is this repository's rule about its own shipped set, and a deployment shipping models owns the same obligation with the same tool, since `modelcheck.check` is importable. A search path usable only unsafely would be a bad feature; one that hands over a duty with the capability is the honest shape. Four perturbations: reading the path at import, searching shipped-first so nearest stops winning, naming only the shipped directory in the refusal, and dropping the merge order -- the last survived the first pass, which was a missing test rather than working code, and the categories test was written to close it |
| `T-102` | done | Make `vrp` an installable distribution | T-101 | NFR-08, CON-6 | `vrp` is 64 modules importable only because `pytest` puts the repo root on `pythonpath`; there is no `[build-system]`, no wheel, and `uv.lock` records the root as `virtual`. Nothing outside this checkout can depend on it at a version. Four modules resolve paths to the **repo root** and read files outside the package -- `servicemodel` (`models/`, now mitigated by `T-101`), `bench/catalogue` (`docs/TDD/scenarios.jsonl`), `bench/comparable` (`benchmarks/instances/`) and `anonymise` (`data/`) -- so an installed copy would find none of them. Delivered as `vrp-platform` 0.3.0 on hatchling. **Renamed**: it was `osrm-api-gateway` while nothing consumed it and the name cost nothing, but shipping the routing platform under a name that says "gateway" would mislead every consumer, and renaming is free exactly once -- before anything pins it. Verified in a clean venv from site-packages with no checkout present: `model`, `servicemodel`, `modelcheck`, `api`, `jobs`, `quote` and `triggers` all import, and a solve returns FEASIBLE and verifier-accepted. The solvers are **extras** -- `pyvrp` is imported at module level by its adapter, `ortools` inside its entry point, and a consumer that only builds and verifies `Problem`s needs neither compiled wheel. `vrp/bench` is **excluded**: it resolves `docs/TDD/scenarios.jsonl` and `benchmarks/instances/` against a repository root an installed copy does not have. `vrp/benchmarks.py` ships, because its reader takes the path from its caller. The repository's own models are **not** in the wheel: they describe *this* repository's operations, and `T-101`'s path is how a deployment describes its own -- an installed copy reports no shipped models rather than pretending ours are anybody's, which is asserted rather than assumed. **The static test earned its place on the first run**: `vrplib` is imported at module level by `vrp/benchmarks.py` and reached the environment only as a transitive dependency of pyvrp, so a reader with nothing to do with pyvrp would have stopped importing the day pyvrp dropped it. Now declared. `uv.lock` moves the root from `virtual` to `editable`; `uv sync --extra dev` was run locally before pushing, because that is the exact command CI runs |
| `T-103` | done | The gate passed models the builder could not use | T-96, T-94 | FR-34, §11.4, AC-4.2 | `T-96`'s promise is that no model reaches `models/` without passing the gate. A model missing a required section passed `structural` with **no complaint at all** and then raised a bare `KeyError` from inside a helper -- a dict key, no model name, no file, the least actionable error this module could produce. `validate_keys` only ever answered the opposite question: it refuses keys that should not be there and never checked for keys that should. **Nine sections, not one**: `assignment`, `fleet`, `name`, `problem_id`, `quantity`, `route`, `service`, `shift`, `windows`. The first was found from outside, by a downstream repository writing its first model against `T-101`'s search path -- which is precisely the reader this gate exists for, and it got a `KeyError`. The fix is in the gate rather than in `validate_keys`: that function's contract is about unknown keys and says so, composition legitimately hands it *fragments*, and widening it broke two contract tests that were right. `missing_sections` is its own function and `modelcheck.structural` calls both. **The list is checked, not trusted.** `test_the_gate_and_the_builder_agree_about_every_key` drops each key of a known-good model in turn and requires the gate to complain *and name it*, or the model to genuinely build -- so a section added to `build` tomorrow and forgotten here fails the suite. A hand-written field list is only defensible when something proves it complete; `COVERS` learned that at 9 of 28. Three perturbations: the gate skipping the check, a key dropping off the list, and the refusal no longer naming the section -- each caught |
| `T-104` | done | The verifier did not check release times | T-04, T-20 | FR-06, INV-17, §11.2, CON-1 | `FR-06` is a MUST -- "an order cannot depart before goods are available" -- and the PyVRP adapter passes `release_time` through, so plans *this* engine builds respect it. Nothing checked it. §11.2 makes the verifier the gate precisely because it shares no code with any solver, so "the solver gets it right" is the one argument it may not accept: a second engine, a hand-built plan, or an integrator's plan arriving through `/verify` had nothing between a vehicle leaving before its load existed and a report calling the plan sound. Sixteen invariants, and none covered a MUST. **`INV-17` binds on the route's departure, not on the stop.** A release time is a fact about the depot -- when the goods are there to be loaded -- so the constraint binds when the vehicle leaves carrying them, which is what `FR-06`'s "cannot depart" says and what the adapter hands PyVRP. Checking it at the stop would excuse a van that left empty-handed and dawdled until the goods existed. Zero means unconstrained, which is the field's default and what every caller already relies on. Found from outside, by a downstream repository whose stages are coupled by exactly this: one leg carries goods to a depot, a later leg dispatches them from it, and the second may not begin before the first has landed. That coupling is a release time, and a single-stage plan never produces one -- which is how sixteen invariants came to exist without it. Three perturbations. Removing the check was caught at once; **checking at the stop and making the bound exclusive both survived the first pass** -- the two points the docstring argued hardest for were the two no fixture could tell apart, because the violating plan failed under either reading. Fixtures rebuilt to discriminate: a route leaving at 0 with goods released at 200 and arriving at 300, and a plan shifted so departure equals release exactly |
| `T-105` | done | An absent edge is not a forbidden one | T-20, T-04 | MTX-5, CON-1, §11.2 | `MTX-5` makes an unreachable pair hard-infeasible rather than expensive, and the PyVRP adapter already honoured it: the edge loop skips the pair and says why. **Omitting an edge does not forbid it.** PyVRP routes through the missing edge and returns the plan with `INFEASIBLE`, so the comment asserted a guarantee the library does not give. The sentinel makes it worse than a missed constraint. `UNREACHABLE = -1`, and in a minimisation that is not "outside the range of any real cost" as `test_uc061`'s reasoning has it -- it is the *most attractive* value in the matrix, so the stranded stop is visited **first**. The gateway's own sentinel is `1e12` (`allocate.rs`), which fails the other way: merely admissible rather than actively optimal. A reader who knows one will misjudge the other. **Severity needs both halves, which is why it survived.** A caller who checks `status` and runs the verifier is protected -- `INFEASIBLE` is returned and `INV-4` fires. It becomes a silent wrong answer only when combined with counting stops without checking status, which is exactly the trap `modelcheck` warns about and which this project walked into on the same day, in `ddn`'s day-one run. Fixed by refusing: `_refuse_unreachable` names a pair and the total before compiling, in the adapter's existing idiom for what it cannot express. `diagnose.preflight` still says *which* order is stranded; this is the backstop for a caller who did not ask. Found on real Costa Rica road data, where disconnected pairs are ordinary rather than pathological. **Misdiagnosed first**: two sessions independently concluded from `grep UNREACHABLE` returning nothing that the adapter had no handling at all. It had handling; the handling did not hold |
