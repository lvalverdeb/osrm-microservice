...
phase_services() {
    ...
    # One shared implementation, plus one thin rc.d wrapper per profile. The
    # wrapper exists only to set ${name} to a literal before sourcing the
    # implementation, so the cycling instance is configured through
    # osrm_routed_cycling_* and stops and starts independently of the others.
    #
    # The name cannot be derived from $0 inside the implementation: /etc/rc
    # sources rc.d scripts rather than executing them, so at boot $0 is the
    # sourcing shell's. An instance named that way looks up an rcvar nobody
    # sets and silently does not start -- see the header of
    # deploy/freebsd/osrm-routed.
    run install -m 444 "${APP_DIR}/deploy/freebsd/osrm-routed" \
        /usr/local/libexec/osrm-routed.rc
    for _entry in $PROFILES; do
        _suffix="${_entry#*:}"; _suffix="${_suffix%%:*}"
        _svc="osrm_routed${_suffix}"
        _wrapper=$(mktemp -t osrm-rcd) || die "mktemp failed"
        cat >"$_wrapper" <<EOF
#!/bin/sh
#
# PROVIDE: ${_svc}
...
# KEYWORD: shutdown
#
# Generated by deploy/freebsd/install.sh -- edit that, not this.
#
# The name is a literal because /etc/rc sources this file rather than running
# it, which makes \$0 the sourcing shell's rather than this script's.
name="${_svc}"
# Redundant with the rcvar= the implementation derives, and stated anyway:
# service(8) -e greps ^name= and ^rcvar= out of the file rather than sourcing
# it, so without this line the engines are missing from the operator's "what
# starts at boot?" listing -- which is how they stayed missing for six days.
rcvar="${_svc}_enable"
. /usr/local/libexec/osrm-routed.rc
EOF
        run install -m 555 "$_wrapper" "/usr/local/etc/rc.d/${_svc}"
        rm -f "$_wrapper"
    done
